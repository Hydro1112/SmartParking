...
class StatsPage(QWidget):
    ...
    def _plot_lines(self, df: pd.DataFrame, d0, d1):
        ...
        # Animation setup
        line_in, = ax.plot([], [], label="Vào")
        line_out, = ax.plot([], [], label="Ra")
        ...
        def animate(i):
            if i >= len(days):
                i = len(days) - 1
            line_in.set_data(days[:i], x_in[:i])
            line_out.set_data(days[:i], x_out[:i])
            ax.relim()
            ax.autoscale_view()
            return line_in, line_out

        ani = FuncAnimation(self.line_fig.fig, animate, init_func=init, frames=len(days)+1, interval=100, blit=True)
        self.line_fig.draw()
    ...
    def _plot_revenue(self, df: pd.DataFrame, d0, d1):
        ...
        def animate(i):
            for bar, y in zip(bars, ys[:i+1]):
                bar.set_height(y)
            ax.relim()
            ax.autoscale_view()
            return bars

        ani = FuncAnimation(self.rev_fig.fig, animate, init_func=init, frames=len(ys), interval=100, blit=True)
        self.rev_fig.draw()
    ...
    def _plot_heat(self, df: pd.DataFrame, d0, d1):
        ...
        # Animation setup
        im = ax.imshow(mat * 0, aspect='auto', origin='lower', alpha=0)
        ...
        def animate(i):
            alpha = min(1.0, i / 10.0)  
            im.set_array(mat * alpha)
            im.set_alpha(alpha)
            return [im]

        ani = FuncAnimation(self.heat_fig.fig, animate, init_func=init, frames=11, interval=100, blit=True)
        self.heat_fig.draw()
